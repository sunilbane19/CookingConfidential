...
function isGarbage(v,names=[]){
 ...
 const low=x.toLowerCase(); if(names.some(n=>{const a=clean(n).toLowerCase();return a&&low===a}))return true;
 ...
 if(/(?:\\|\\|\\|\\|\\|)/.test(x)||/(?:\\\\|~){2,}/.test(x))return true;
 return false;
}
function normaliseRecipe(r,names){
 const out={...r}; out.name=clean(r.name)||'Imported recipe';
 let rawIng=Array.isArray(r.ingredients)?r.ingredients:cleanLines(r.ingredients);
 let rawMethod=Array.isArray(r.method)?r.method:(r.method||r.instructions||r.directions||'');
 const ing=[],met=[]; let methodStarted=false;
 for(const raw of rawIng){
  const x=ingredientText(raw); if(!x)continue;
  if(!methodStarted && /^(method|directions?|instructions?|preparation|steps?|procedure)\\s*[:\\-–—]?$/i.test(x)){methodStarted=true;continue;}
 ...
 for(const raw of (Array.isArray(rawMethod)?rawMethod:cleanLines(rawMethod))){
  const x=clean(raw); if(!x||heading.test(x)||isGarbage(x,names))continue; met.push(x);
 }
...
function extractRecipes(x){
 ...
 const names=rs.map(r=>clean(r.name)).filter(Boolean);
...
async function saveRecipe(id,x,r){
 ...
 const payload={name:clean(r.name),description:clean(r.description)||null,cuisine:clean(r.cuisine)||null,course:clean(r.course)||null,recipe_type:clean(r.recipe_type)||'Marinade',servings:clean(r.servings)||null,ingredients:r.ingredients.map(ingredientText).filter(Boolean),method:r.method.map(clean).filter(Boolean).join('\\n'),source_type:x.source_url?'social':'file',source_url:x.source_url||null,source_title:x.source_url?(x.source_title||null):(x.file_name||null),created_by:user.id,visibility:'private'};
...
function editOne(id,x,recipes,i,saved){
 ...
 dialog.querySelector('#ccMultiOne').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,fd=new FormData(f);const updated={...r,name:clean(fd.get('name')),cuisine:clean(fd.get('cuisine')),course:clean(fd.get('course')),servings:clean(fd.get('servings')),ingredients:cleanLines(fd.get('ingredients')).map(ingredientText).filter(Boolean),method:cleanLines(fd.get('method')),notes:cleanLines(fd.get('notes'))};const btn=f.querySelector('[type="submit"]');btn.disabled=true;btn.textContent='Saving…';try{await saveRecipe(id,x,updated);recipes[i]=updated;saved.add(i);renderList(id,x,recipes,saved)}catch(err){btn.disabled=false;btn.textContent='Save this recipe';alert(err.message||'Could not save recipe.')}};
}
