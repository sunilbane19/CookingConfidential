...
// Import review recovery: if an item is still pending, run extraction when the
// user taps Review, then render the extracted recipe. This also avoids stacking
// the review dialog on top of the import dialog on mobile browsers.
...
function parseRecipe(x) {
  let recipe = {
    name: x.file_name?.replace(/\.[^.]+$/, '') || 'Imported recipe',
    ingredients: [], method: '', cuisine: x.inferred_cuisine || '',
    course: x.inferred_course || '', servings: ''
  };
  if (x.extraction_status === 'ready' && x.extracted_text) {
    try {
      ...
      recipe = {
        ...recipe,
        name: j.name || recipe.name,
        ingredients: j.recipeIngredient || [],
        method: Array.isArray(j.recipeInstructions)
          ? j.recipeInstructions.map(v => typeof v === 'string' ? v : v.text || v.name || '').join('\n')
          : j.recipeInstructions || '',
        cuisine: j.recipeCuisine || recipe.cuisine,
        course: j.recipeCategory || recipe.course,
        servings: j.recipeYield || ''
      };
    } catch (_) {}
  }
  return recipe;
...
function showReview(x, id) {
  ...
  detailDialog.querySelector('#detailContent').innerHTML = `<button class="close" onclick="detailDialog.close()">×</button>
    ...
    <form id="ccImportReviewForm">
      ...
      <div class="detail-actions"><button class="primary" type="submit">Save to recipes</button></div>
    </form>`;
  detailDialog.showModal();

  document.querySelector('#ccImportReviewForm').onsubmit = async e => {
    e.preventDefault();
    ...
    const { error } = await supabase.from('cc_recipes').insert({
      ...
      servings: f.get('servings') || null, ingredients, method: f.get('method') || null,
      source_type: x.source_url ? 'social' : 'file', source_url: x.source_url || null,
      source_title: x.source_title || x.file_name || null, created_by: user.id, visibility: 'private'
    });
    if (error) return alert(error.message);
    const { error: updateError } = await supabase.from('cc_import_items')
      .update({ review_status: 'approved', extraction_status: 'ready' }).eq('id', id);
...
async function reviewImportFixed(id) {
  ...
  if (x.extraction_status === 'pending' || x.extraction_status === 'processing') {
    ...
    if (fxError) {
      detailDialog.querySelector('#detailContent').innerHTML = `<button class="close" onclick="detailDialog.close()">×</button><p class="eyebrow">IMPORT ERROR</p><h2>Recipe extraction failed</h2><p class="small-note">${esc(fxError.message || 'The recipe could not be extracted. Please try again.')}</p>`;
      return;
    }
  }
  ...
  if (latestError) {
    detailDialog.close();
    return alert(latestError.message);
  }
  if (latest.extraction_status === 'failed') {
    detailDialog.querySelector('#detailContent').innerHTML = `<button class="close" onclick="detailDialog.close()">×</button><p class="eyebrow">IMPORT ERROR</p><h2>Recipe extraction failed</h2><p class="small-note">${esc(latest.error_message || 'The recipe could not be extracted from this file.')}</p>`;
    return;
  }
...
document.addEventListener('click', event => {
  ...
  if (!button) return;
  // The original app handler is attached directly to the button. Capture here
  // so the recovery flow runs instead of the old blank-field flow.
  event.preventDefault();
  event.stopImmediatePropagation();
  reviewImportFixed(Number(button.dataset.id));
}, true);
