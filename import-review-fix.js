...
const cleanRefs = (value = '') => String(value).replace(/\s*\[[\s\d,;,-]+\]\s*/g, ' ').replace(/\s+/g, ' ').trim();
const cleanMethod = (value = '') => String(value).replace(/\s*\[[\s\d,;,-]+\]\s*/g, ' ').replace(/[ \t]+\n/g, '\n').replace(/\n{3,}/g, '\n\n').trim();
...
function parseRecipe(x) {
  ...
  recipe.name=cleanRefs(recipe.name); recipe.description=cleanRefs(recipe.description); return recipe;
}

async function showOriginalFile(x) {
  if (!x.file_path) return;
  const {data, error} = await supabase.storage.from('cooking-confidential').createSignedUrl(x.file_path, 300);
  if (error || !data?.signedUrl) return alert(error?.message || 'Could not open the original file.');
  const res = await fetch(data.signedUrl);
  if (!res.ok) return alert('Could not load the original file.');
  const blob = await res.blob();
  const name = x.file_name || 'Original recipe';
  const lower = name.toLowerCase();
  const back = `<button class="close" type="button" id="ccBackToReview">×</button><p class="eyebrow">ORIGINAL RECIPE</p><h2>${esc(name)}</h2><p class="small-note">Original uploaded file. It remains private and is not exposed as a public URL.</p>`;
  let viewer = '';
  if (lower.endsWith('.docx')) {
    const buffer = await blob.arrayBuffer();
    const result = await mammoth.convertToHtml({arrayBuffer: buffer});
    viewer = `<div class="original-viewer">${result.value || '<p>No readable content found.</p>'}</div>`;
  } else if (lower.endsWith('.pdf') || blob.type === 'application/pdf') {
    const url = URL.createObjectURL(blob);
    viewer = `<iframe class="original-pdf" title="Original recipe PDF" src="${url}"></iframe>`;
  } else if (blob.type.startsWith('image/')) {
    const url = URL.createObjectURL(blob);
    viewer = `<div class="original-viewer"><img class="original-image" alt="Original recipe" src="${url}"></div>`;
  } else {
    viewer = `<pre class="original-viewer">${esc(await blob.text())}</pre>`;
  }
  detailDialog.querySelector('#detailContent').innerHTML = `${back}${viewer}`;
...
function showReview(x,id){
  ...
  const originalLabel = x.file_name ? `View original: ${x.file_name}` : 'View original';
  ...
  const originalButton=document.querySelector('#ccViewOriginal'); if(originalButton) originalButton.onclick=()=>showOriginalFile({...x,id});
  form.onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);const{data:{user}}=await supabase.auth.getUser();if(!user)return alert('Please sign in again before saving the recipe.');const chosenCourse=String(f.get('course')==='__custom__'?f.get('custom_course'):f.get('course')).trim();const ingredients=String(f.get('ingredients')).split('\n').map(v=>cleanRefs(v.trim())).filter(Boolean);const payload={name:cleanRefs(f.get('name')),description:cleanRefs(f.get('description')||'')||null,cuisine:f.get('cuisine')||null,course:chosenCourse||null,servings:f.get('servings')||null,ingredients,method:cleanMethod(f.get('method')||''),source_type:x.source_url?'social':'file',source_url:x.source_url||null,source_title:x.source_url?x.source_title:x.file_name||null,created_by:user.id,visibility:'private'};
    let existingId=x.recipe_id||null;if(!existingId&&x.file_name){const{data:matches}=await supabase.from('cc_recipes').select('id').eq('created_by',user.id).eq('source_type','file').eq('source_title',x.file_name).order('id',{ascending:true}).limit(1);if(matches?.length)existingId=matches[0].id;}
