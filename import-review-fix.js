...
function parseRecipe(x) {
  ...
  if(x.extraction_status==='ready'&&x.extracted_text){
    try{
      const j=JSON.parse(x.extracted_text); const extractedName=String(j.name||'').trim();
      const longIntro=extractedName.length>100||/\b(we will|here is|to make|using|ingredients list)\b/i.test(extractedName);
      recipe={...recipe,name:longIntro?fileTitle:(extractedName||fileTitle),ingredients:j.ingredients||j.recipeIngredient||[],method:j.method||j.recipeInstructions||'',cuisine:j.cuisine||j.recipeCuisine||recipe.cuisine,course:j.course||j.recipeCategory||recipe.course,servings:j.servings||j.recipeYield||'',description:j.description||(longIntro?extractedName:'')};
...
function showReview(x,id){
  ...
  const form=document.querySelector('#ccImportReviewForm'); const category=form.querySelector('[name="course"]'); const custom=form.querySelector('[name="custom_course"]'); const known=[...category.options].map(o=>o.value).filter(v=>v&&v!=='__custom__');
  if(selectedCourse&&known.includes(selectedCourse))category.value=selectedCourse; else if(selectedCourse){category.value='__custom__';custom.value=selectedCourse;custom.style.display='block';}
