...
function fieldMarkup(f){
  ...
  return `<label${cls}>${esc(f.label)}<input type="${esc(f.type||'text')}" name="${esc(f.name)}" value="${esc(value)}"${required}${f.placeholder?` placeholder="${esc(f.placeholder)}"`:''}></label>`;
}

function groupMarkup(group){return `<div class="two-col">${group.map(fieldMarkup).join('')}</div>`;}
...
export function createGenericEditor({dialog,eyebrow='EDIT',title='Edit',sourceHtml='',fields=[],actions={},onSave}){
  ...
  const body=[];fields.forEach(f=>body.push(f.group?groupMarkup(f.group):fieldMarkup(f)));
  ...
  content.scrollTop=0;dialog.showModal();content.scrollTop=0;
  const form=content.querySelector('#ccGenericEditorForm');
  const close=()=>dialog.close();content.querySelector('#ccGenericEditorClose').onclick=close;content.querySelector('#ccGenericEditorCancel').onclick=close;
  if(actions.delete&&actions.onDelete)content.querySelector('#ccGenericEditorDelete').onclick=()=>actions.onDelete();
  content.querySelectorAll('select').forEach(sel=>{const custom=form.querySelector(`[name="${sel.name}_custom"]`);if(!custom)return;const sync=()=>{custom.style.display=sel.value==='__custom__'?'block':'none';if(sel.value!=='__custom__')custom.value=''};sel.addEventListener('change',sync);sync()});
  content.querySelectorAll('.rich-editor').forEach(editor=>{const name=editor.dataset.richName,hidden=form.querySelector(`textarea[name="${CSS.escape(name)}"]`);const sync=()=>{if(hidden)hidden.value=sanitizeRichHtml(editor.innerHTML)};editor.addEventListener('input',sync);editor.addEventListener('blur',sync);editor.querySelectorAll('a').forEach(a=>a.removeAttribute('href'));editor.parentElement.querySelectorAll('.rich-tool').forEach(button=>{button.addEventListener('mousedown',e=>e.preventDefault());button.addEventListener('click',()=>{editor.focus();document.execCommand(button.dataset.command,false,null);sync()})})});
