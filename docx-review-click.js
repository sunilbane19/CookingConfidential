...
document.addEventListener('click',async event=>{const button=event.target?.closest?.('.cc-inbox-review');if(!button||button.dataset.image==='1'||button.dataset.docx!=='1')return;const id=Number(button.dataset.id);if(!id)return;event.preventDefault();event.stopImmediatePropagation();try{const mod=await import('./docx-import-review.js?v=1.0.6');if(typeof mod.reviewDocxImport!=='function')throw Error('The DOCX review module is missing or incomplete.');await mod.reviewDocxImport(id);}catch(e){document.querySelector('#detailDialog')?.close();showDocxReviewError(e?.message||'Could not open the DOCX review.');}},true);
