...
function tidyLines(value){
 ...
 for(let x of a){
  ...
  x=x.replace(/^1\s+tbsp\s+2\s+ginger,?\s+water\s+grated$/i,'1 tbsp ginger, grated').replace(/^1\s+b\s+ack\s+pepper$/i,'½ tsp black pepper').replace(/^cup\s+Rbs\s+brown\s+sugar$/i,'').replace(/^sesame\s+oil$/i,'').replace(/^pepper$/i,'').replace(/^1\s+top\s+black\s+pepper$/i,'1 tsp black pepper').replace(/^1\s+tsp\s+grou\s*nd\s+A\s*08$/i,'1 tsp ground cumin');
