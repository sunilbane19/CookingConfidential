...
function tidyLines(value){
 ...
 for(let x of a){
  ...
  x=x.replace(/^4\s+1\s+tbsp\s+soy\s+sauce$/i,'1 tbsp soy sauce').replace(/^cloves\s+ga\s*ic\.?$/i,'4 cloves garlic, minced').replace(/^1t\s+sesame oil$/i,'1 tsp sesame oil').replace(/^1\s+tbsp\s+sesame\s+oll$/i,'1 tbsp sesame oil').replace(/^3\s+tbs\s+Soy$/i,'3 tbsp soy sauce').replace(/^2\s+tbsp\s+Worce(?:\s+e)?$/i,'2 tbsp Worcestershire sauce').replace(/^tbsp\s+Worce(?:\s+e)?$/i,'2 tbsp Worcestershire sauce');
  x=x.replace(/^2\s+3\s+tbsp\s+Soy$/i,'3 tbsp soy sauce').replace(/^3\s+cloves\s+garlic,\s*ed$/i,'3 cloves garlic, minced').replace(/^tsp\s+chili\s+pav[-–]?des$/i,'1 tsp chili powder').replace(/^1\s+tsp\s+black\s+oregano$/i,'1 tsp oregano').replace(/^1\s+tsp\s+tsp\s+black\s+oregano$/i,'1 tsp oregano').replace(/^1\s+tsp\s+grou\s*nd$/i,'1 tsp ground cumin').replace(/^1\s+tbsp\s+toasted\s+sesame\s*[.]?$/i,'1 tbsp toasted sesame seeds').replace(/^½\s+cup\s+lime\]$/i,'½ cup lime juice');
  x=x.replace(/^1\s+tbsp\s+2\s+ginger,?\s+water\s+grated$/i,'1 tbsp ginger, grated').replace(/^1\s+b\s+ack\s+pepper$/i,'').replace(/^cup\s+Rbs\s+brown\s+sugar$/i,'').replace(/^sesame\s+oil$/i,'').replace(/^pepper$/i,'').replace(/^1\s+top\s+black\s+pepper$/i,'1 tsp black pepper').replace(/^1\s+tsp\s+grou\s*nd\s+A\s*08$/i,'1 tsp ground cumin');
  x=x.replace(/^tbsp\s+sesame\s+ol$/i,'1 tbsp sesame oil').replace(/^_1tbsp\s+toasted\s+sesame\s+seed$/i,'1 tbsp toasted sesame seeds').replace(/^½\s+cup\s+olive$/i,'½ cup olive oil').replace(/^3\s+tbs\s+Soy$/i,'3 tbsp soy sauce').replace(/^2\s+tbsp\s+Worce$/i,'2 tbsp Worcestershire sauce').replace(/^3\s+cloves\s+garlic,$/i,'3 cloves garlic, minced').replace(/^1\s+tbsp\s+toasted\s+sesame\s*\.$/i,'1 tbsp toasted sesame seeds').replace(/^1¥%2\s+chili$/i,'1½-2 lbs beef of choice');
 ...
 for(let i=0;i<out.length;i++){if(/^1\s+tsp\s+black$/i.test(out[i])&&/^pepper$/i.test(out[i+1]||'')){out[i]='1 tsp black pepper';out.splice(i+1,1);i--;continue;}if(/^3\s+cloves\s+garlic,?$/i.test(out[i])&&/^3\s+cloves\s+garlic,?\s+(?:ed|minced)$/i.test(out[i+1]||'')){out[i]=out[i+1];out.splice(i+1,1);}}
 if(out.some(x=>/^1\s+tsp\s+black\s+pepper$/i.test(x)))out=out.filter(x=>!/^½\s+tsp\s+black\s+pepper$/i.test(x));
