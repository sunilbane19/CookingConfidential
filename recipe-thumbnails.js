...
function candidates(r){const chosen=String(r?.image_url||'').trim();const t=textOf(r);const match=IMAGE_POOL.find(x=>x.keys.test(t));const id=Number(r?.id)||0;const fallback={url:DEFAULT_IMAGES[Math.abs(id)%DEFAULT_IMAGES.length],alt:'Food photograph'};if(chosen)return[{url:chosen,alt:'Recipe image'},...(match?[match]:[]),fallback];return match?[match,fallback]:[fallback]}
