...
async function loadRecipePage(){
  ...
  let q=supabase.from('cc_recipes').select('*',{count:'exact'}).order('updated_at',{ascending:false}).range(recipePage*PAGE_SIZE,recipePage*PAGE_SIZE+PAGE_SIZE-1);
  const f=searchFilter(search?.value);if(f)q=q.or(f);if(view==='favourites')q=q.eq('is_favourite',true);
  const timeout=new Promise((_,reject)=>setTimeout(()=>reject(new Error('The recipe library is taking too long to respond. Please try again.')),12000));
  const result=await Promise.race([q,timeout]);
  const {data,error,count}=result;if(error)throw new Error(error.message);recipes=data||[];recipeTotal=count||0;render()
}
