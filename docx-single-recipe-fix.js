...
function renderReview(dialog,id,item,recipe){
  ...
  d.querySelector('#ccSingleRecipe').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget);recipe={...recipe,name:clean(f.get('name')),ingredients:clean(String(f.get('ingredients')||'')).split('\n').map(clean).filter(Boolean),method:clean(String(f.get('method')||'')).split('\n').map(clean).filter(Boolean),notes:clean(String(f.get('notes')||'')).split('\n').map(clean).filter(Boolean)};await saveSingle(dialog,id,item,recipe);};
}
...
async function fixAfterExtraction(id){
  const dialog=document.querySelector('#detailDialog');if(!dialog)return;
  const item=await getItem(id);if(!/\.docx$/i.test(item.file_name||''))return;
  let payload;try{payload=JSON.parse(item.extracted_text||'{}');}catch{return;}
  const recipes=Array.isArray(payload.recipes)?payload.recipes:[];
  if(recipes.length<2||!recipes.every(r=>COMPONENT.test(clean(r.name))))return;
  ...
  await sb.from('cc_import_items').update({extracted_text:JSON.stringify({version:6,multiple:false,recipes:[merged]}),source_title:merged.name,extraction_status:'ready',review_status:'pending',error_message:null}).eq('id',id);
