...
import { createGenericEditor, editorValue } from './generic-editor.js?v=1.0.0';
...
async function source(r){
  ...
  if(!x){const recent=await supabase.from('cc_import_items').select('file_path,file_name,source_url,source_title,extracted_text').order('id',{ascending:false}).limit(50);if(!recent.error){const target=String(r.name||'').trim().toLowerCase();x=(recent.data||[]).find(item=>{try{const p=JSON.parse(item.extracted_text||'');const rs=Array.isArray(p?.recipes)?p.recipes:[];return rs.some(recipe=>String(recipe?.name||'').trim().toLowerCase()===target)}catch{return false}})||null;}}
...
async function openOriginal(r,x){
  if(!x?.file_path)return;const u=await supabase.storage.from('cooking-confidential').createSignedUrl(x.file_path,300);if(u.error||!u.data?.signedUrl)return window.ccShowError(u.error?.message||'Could not open original file.','Could not open original file');
  const name=String(x.file_name||'Original recipe'),ext=name.split('.').pop()?.toLowerCase()||'',isPdf=ext==='pdf',isImage=/^(png|jpe?g|webp|gif|bmp|avif)$/i.test(ext);
...
async function openEditor(r){
  const s=await source(r);
  const editor=createGenericEditor({dialog,eyebrow:'EDIT RECIPE',title:'Update recipe',sourceHtml:s.html,fields:[
    {label:'Recipe name',name:'name',value:r.name,required:true},
    {group:[{label:'Cuisine',name:'cuisine',value:r.cuisine},{label:'Course',name:'course',value:r.course,type:'select',options:courses,allowCustom:true}]},
  ...
  editor.form.querySelector('#ccViewOriginalEdit')?.addEventListener('click',()=>openOriginal(r,s.item));
}
