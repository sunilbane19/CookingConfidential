...
async function openOriginal(r,x){
  ...
  if(!isPdf&&!isImage){
    dialog.querySelector('#detailContent').innerHTML=`<button class="close" type="button" id="ccOriginalEditBack">×</button><p class="eyebrow">ORIGINAL RECIPE</p><h2>${esc(name)}</h2><p class="small-note">This file type cannot be previewed inside the browser.</p><p><a class="primary" href="${esc(u.data.signedUrl)}" target="_blank" rel="noopener noreferrer">Open original file</a></p>`;
...
async function openEditor(r){
  ...
  dialog.showModal();
  const form=document.querySelector('#ccEditForm');
