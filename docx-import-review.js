...
// A DOCX often uses culinary component headings inside one recipe. These are
// not recipe names and must not cause the document to be split into recipes.
...
function parseSection(nodes,start,end){
 const r=makeRecipe(nodes[start]?.text||'');
 const section=nodes.slice(start+1,end);let sawIngredient=false,afterIngredients=false;
...
function mergeComponentRecipe(nodes,heads,fileName){
 ...
 for(let i=0;i<relevant.length;i++){
  const start=relevant[i],end=i+1<relevant.length?relevant[i+1]:nodes.length,r=parseSection(nodes,start,end);
  merged.ingredients.push(...r.ingredients);merged.method.push(...r.method);merged.notes.push(...r.notes);
 }
 return merged;
}
function parseDocx(html,fileName){
 ...
 // If there is one normal title and the remaining detected headings are
 // culinary components, or if every detected heading is a component, this is
 // one recipe. This keeps the rule generic for future DOCX imports.
 ...
 if(componentHeads.length&&normalHeads.length<=1){
  const merged=mergeComponentRecipe(nodes,recipeHeads,fileName);
  if(merged.ingredients.length||merged.method.length)return[merged];
 }
