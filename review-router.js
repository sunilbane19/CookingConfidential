...
async function loadReviewModules(){
  const [single,multi,fixed,ocr,cleanup]=await Promise.all([
    ...
    import('./import-review-cleanup.js?v=1.0.0')
  ]);
...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    if(recipeCount===1&&await modules.single(id)){await modules.cleanup(id);return;}
    if(recipeCount>1&&await modules.multi(id)){await modules.cleanup(id);return;}
