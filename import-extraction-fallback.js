...
function parse(text,file){
  ...
  const heading=(x,n)=>n.some(v=>new RegExp('^'+v+'\\s*:?[\\s]*$','i').test(x));
  const ii=lines.findIndex(x=>heading(x,ingHeads)), mi=lines.findIndex(x=>heading(x,methHeads));
  const next=(i)=>{const a=[ii,mi].filter(x=>x>i);return a.length?Math.min(...a):lines.length;};
  let ingredients=ii>=0?lines.slice(ii+1,next(ii)).filter(x=>x.length>1):[];
  if(!ingredients.length) ingredients=lines.filter(x=>/^(?:[-•·]\s*)?(?:\d+(?:[./]\d+)?|½|⅓|⅔|¼|¾)\s*(?:cups?|tbsp|tbs|tsp|g|kg|mg|ml|l|oz|lb|cloves?|slices?|pieces?|medium|large|small)\b/i.test(x));
  let method=mi>=0?lines.slice(mi+1).join('\n'):lines.filter(x=>/^(?:step\s*\d+|\d+[.)])\s+/i.test(x)).join('\n');
  let name=clean(lines[0]||file.replace(/\.[^.]+$/,''));
  if(name.length>160||/\b(we will|here is|to make|using|ingredients list)\b/i.test(name)){const c=lines.find(x=>x.length>2&&x.length<120&&!heading(x,ingHeads)&&!heading(x,methHeads));if(c)name=c;}
  return {name,description:'',cuisine:'',course:'',recipe_type:'Dish',servings:'',ingredients,method:clean(method)};
}
function opts(values,groups=false){return values.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join('');}
async function extractAndShow(id){
  ...
  try{
    ...
    const {data:blob,error:be}=await sb.storage.from('cooking-confidential').download(x.file_path);
    if(be||!blob) throw Error(be?.message||'Could not read the original file.');
    let text='';
    if(/\.docx$/i.test(x.file_name||'')){const r=await mammoth.extractRawText({arrayBuffer:await blob.arrayBuffer()});text=r.value||'';}
    ...
    if(!text.trim()) throw Error('The original file contained no readable text.');
    const recipe=parse(text,x.file_name||'Imported recipe');
    const {error:ue}=await sb.from('cc_import_items').update({extracted_text:JSON.stringify(recipe),source_title:recipe.name,extraction_status:'ready',review_status:'pending',inferred_cuisine:null,inferred_course:recipe.course,error_message:null}).eq('id',id);
    if(ue) throw ue;
...
function renderReview(x,id){
  ...
  detail.querySelector('#ccFallbackCancel').onclick=()=>detail.close();
  detail.querySelector('#ccFallbackOriginal').onclick=()=>showOriginal(x);
}
