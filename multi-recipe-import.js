...
async function recognizeTile(worker,bitmap,x,y,w,h){const canvas=document.createElement('canvas');canvas.width=Math.round(w*2);canvas.height=Math.round(h*2);const ctx=canvas.getContext('2d');ctx.drawImage(bitmap,x,y,w,h,0,0,canvas.width,canvas.height);return worker.recognize(canvas);}
async function ocrImage(blob,status){status('Reading image…');const worker=await createWorker('eng',1,{logger:m=>{if(m.status==='recognizing text')status(`Reading image… ${Math.round((m.progress||0)*100)}%`);}});try{const full=await worker.recognize(blob);let recipes=buildFromLines(ocrLines(full.data.words||[]));const bitmap=await createImageBitmap(blob),W=bitmap.width,H=bitmap.height,ratio=W/H;if(ratio<.85||recipes.length<4){const grids=ratio>.9&&ratio<1.1?[[3,3],[2,2],[3,2]]:ratio<.75?[[3,3],[2,3],[3,4],[2,4]]:[[3,2],[2,2],[4,2],[3,3]];for(const [cols,rows] of grids){const found=[],tw=W/cols,th=H/rows;for(let ry=0;ry<rows;ry++)for(let cx=0;cx<cols;cx++){const res=await recognizeTile(worker,bitmap,cx*tw,ry*th,tw,th),tileRecipes=buildFromLines(ocrLines(res.data.words||[]));if(tileRecipes.length)found.push(...tileRecipes);}const unique=dedupeRecipes(found);if(unique.length>recipes.length)recipes=unique;if(recipes.length>=6)break;}bitmap.close();}else bitmap.close();if(!recipes.length){const r=makeRecipe('Imported image');r.ingredients=cleanIngredientLines(ocrLines(full.data.words||[]).map(x=>x.text));recipes=[r];}return recipes;}finally{await worker.terminate();}}
